    ...
    <div className="bg-jarvis-bar/60 border-b border-jarvis-border px-4 py-2 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-[12px] opacity-45" style={{ letterSpacing: '1px' }}>MODE</span>
          <span className={`text-[14px] font-bold ${modeColor}`} style={{ letterSpacing: '2px' }}>
            {masterMode}
          </span>
          <span className="text-[13px] opacity-60">{subMode}</span>
        </div>

        {tactical?.nav?.autothrust && (
          <span className="text-[12px] text-jarvis-success" style={{ letterSpacing: '1px' }}>
            A/T ON
          </span>
        )}
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="flex items-center gap-2">
          <span className="text-[12px] opacity-45" style={{ letterSpacing: '1px' }}>TGT</span>
          <span className="text-[13px] font-bold text-jarvis-accent tabular-nums">{targetCount}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[12px] opacity-45" style={{ letterSpacing: '1px' }}>LOCK</span>
          <span className={`text-[13px] font-bold tabular-nums ${lockedCount > 0 ? 'text-jarvis-danger glow-danger' : 'text-jarvis-muted'}`}>
            {lockedCount}
          </span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[12px] opacity-45" style={{ letterSpacing: '1px' }}>OBJ</span>
      ...
      <div className="flex items-center gap-3">
        {!hasPermObjects && (
          <span className="text-[12px] text-jarvis-warning opacity-60" style={{ letterSpacing: '1px' }}>
            OBJ GATED
          </span>
        )}
        {!hasPermSensors && (
        ...
        <span className={`text-[12px] ${connectionColor}`}>
